...
import type { Vehicle, VehicleStatus } from '../types/database.types';
      ...
      if (currentVehicle.id) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            status: currentVehicle.status as VehicleStatus,
          })
              ...
              <tbody className="bg-white dark:bg-slate-900/10 divide-y divide-slate-200/80 dark:divide-slate-800/50">
                {filteredVehicles.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-900 dark:text-white">{v.name}</td>
